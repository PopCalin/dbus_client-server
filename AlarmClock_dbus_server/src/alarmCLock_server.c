...
{
	/* Check the argments */
	if (IS_VALID_HOUR_ARGS(arg_sec) && IS_VALID_HOUR_ARGS(arg_min) && IS_VALID_HOUR_ARGS(arg_hour) && (arg_week_day <= 127 && arg_week_day >= 1))
	{
...
void *thread_function()
{
	while (TRUE)
	{
		...
		unsigned char server_weekday = 1 << (g_date_time_get_day_of_week(server_time) - 1);
		unsigned char is_weekday_ok  = server_weekday & alarm_day;
...
int main (int argc, char *argv[])
{
  ...
  g_bus_own_name (G_BUS_TYPE_SYSTEM, "com.example.AlarmService", G_BUS_NAME_OWNER_FLAGS_REPLACE, on_bus_acquired, on_name_acquired, on_name_lost, NULL, NULL);
